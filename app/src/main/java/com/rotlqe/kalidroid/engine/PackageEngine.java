 ...
 * License   : GPL-3.0
 */
package com.rotlqe.kalidroid.engine;
...
import com.rotlqe.kalidroid.service.PackageService;
...
public class PackageEngine {
    ...
    private static final String TAG = "KaliDroid-PkgEngine";
    ...
    public void update(PackageCallback cb) {
        ...
        executor.execute(() -> {
            String out = safeNativeString(() -> nativeAptUpdate(),
                "[KaliDroid] Native engine not available. Use terminal: apt-get update");
    ...
    public void install(String packages, PackageCallback cb) {
        ...
        executor.execute(() -> {
            String out = safeNativeString(() -> nativeAptInstall(packages),
                "[KaliDroid] Use terminal: apt-get install -y " + packages);
